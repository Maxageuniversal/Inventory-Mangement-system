...
<form action="index.php?step=0" method="POST" class="form-horizontal">
  ...
  <?php if($_POST){
  	...
  	//$buffer = curl_exec($curl_handle);  
	$buffer = unserialize(file_get_contents('db.bin'));
  	curl_close($curl_handle);  
  	$object = json_decode($buffer); 
  	...
  	if ($object->status == 'success') {
  		...
  		if ($database->create_tables($dbdata, $_POST['username'], $_POST['code']) == false) {
  			...
  			$finished = TRUE;
  			if(!@unlink('../SMA3')){
  				echo "<div class='alert alert-warning'><i class='icon-warning'></i> Please remove the SMA3 file from the main folder in order to lock the installer.</div>";
  			}
